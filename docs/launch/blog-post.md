...
| **False positive rate** | Not published | ~78% on YARA rules | 15,248 → 90 findings after tuning on 15 production servers, all 90 manually confirmed |

Honest caveat: we validated on 15 production servers -- initial untuned rules produced 15,248 findings, tuned rules produce 90, and we hand-checked all 90 as real. That is a 99.4% cut in finding volume on that corpus, not a measured false positive rate for the tool in general. We still don't have a standardized benchmark corpus -- if you know of one, open an issue.
...
- **Further FP reduction** -- 15,248 → 90 findings on 15 production servers, but that is one corpus and more tuning is needed
