...
**Subject:** Open-source tool reads MCP server source code to find RCE — found 4 across 106 public servers
...
Key facts:
- Scanned 106 public MCP servers, found 4 confirmed RCE (the largest had 25K GitHub stars)
...
Key facts:
- 106 public MCP servers scanned, 4 had confirmed remote code execution
...
**Subject:** mcp-redteam — open-source security scanner for MCP servers (v0.5.2)
...
- Found 4 confirmed RCE vulnerabilities across 106 public MCP servers
- Works fully local in deterministic mode, no cloud dependency
- 217 tests including fuzzing; ran its own scanner on its own code
...
Key facts:
- GitHub Action: `uses: m0rvayne/mcp-redteam@v0.5.2` with `fail-on: critical`
...
- Self-tested: 217 tests + published self-security audit
...
Key facts:
- Scanned 106 public MCP servers, found 4 with confirmed remote code execution
